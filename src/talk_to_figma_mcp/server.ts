...
// Note: Removed global currentChannel to support multi-user remote deployment
// Each request now specifies its channel explicitly
...
const serverUrl = serverArg ? serverArg.split('=')[1] : 'talk-to-figma-socket.onrender.com';
...
server.tool(
  ...
  async ({ nodeIds, channel }: any) => {
    try {
      const result = await sendCommandToFigma("delete_multiple_nodes", { nodeIds }, channel);
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  async ({ nodeId, annotationId, labelMarkdown, categoryId, properties, channel }: any) => {
    try {
...
server.tool(
  ...
  async ({ nodeId, paddingTop, paddingRight, paddingBottom, paddingLeft, channel }: any) => {
    try {
...
server.tool(
  ...
  async ({ nodeId, primaryAxisAlignItems, counterAxisAlignItems, channel }: any) => {
    try {
...
server.tool(
  ...
  async ({ nodeId, layoutSizingHorizontal, layoutSizingVertical, channel }: any) => {
    try {
...
server.tool(
  ...
  async ({ nodeId, itemSpacing, counterAxisSpacing, channel }: any) => {
    try {
      ...
      const result = await sendCommandToFigma("set_item_spacing", params, channel);
...
server.tool(
  ...
  async ({ nodeIds, channel }: any) => {
    try {
      const result = await sendCommandToFigma("get_reactions", { nodeIds }, channel);
      return {
        content: [
          {
            type: "text",
...
type CommandParams = {
  ...
  get_styles: { channel: string; };
  get_local_components: { channel: string; };
  ...
  get_reactions: { nodeIds: string[]; channel: string; };
  set_default_connector: {
...
// Function to join a channel (no longer stores globally)
